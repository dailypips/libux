...
#define LDOUBLE_MIN_10_EXP __LDBL_MIN_10_EXP__
#define LDOUBLE_MAX_10_EXP __LDBL_MAX_10_EXP__
#else
#define LDOUBLE double
#define LDOUBLE_MIN_10_EXP __DBL_MIN_10_EXP__
#define LDOUBLE_MAX_10_EXP __DBL_MAX_10_EXP__
...
#ifdef UX_TIME_PRINTF
#include <stdint.h>
static void fmttime(char*, size_t *, size_t, uint64_t, int, int, int, int);
#endif
	...
	while (ch != '\0')
		...
		case PRINT_S_CONV:
			...
			case '%':	/* Print a "%" character verbatim. */
				...
				break;
#ifdef UX_TIME_PRINTF
            case 't':   /* Print ux_time_t */
                value = va_arg(args, uint64_t);
                fmttime(str, &len, size, value, base, width,
                    precision, flags);
#endif
...
#ifdef UX_TIME_PRINTF
static void
fmttime(char *str, size_t *len, size_t size, uint64_t value, int width,
       int precision, int flags)
{
    *len += ux_time_format(str, size, value, 0);
}
#endif
